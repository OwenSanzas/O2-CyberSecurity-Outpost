    ...
    <article className="max-w-3xl mx-auto px-4 pt-6 pb-16">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {onPrev && (
            <button onClick={onPrev} className="w-8 h-8 flex items-center justify-center rounded-lg bg-[var(--color-bg-card)] border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:border-[var(--color-accent)]/30 transition-all cursor-pointer" title="Previous (←)">
              ←
            </button>
          )}
          ...
          {onNext && (
            <button onClick={onNext} className="w-8 h-8 flex items-center justify-center rounded-lg bg-[var(--color-bg-card)] border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:border-[var(--color-accent)]/30 transition-all cursor-pointer" title="Next (→)">
              →
            </button>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        {paper.venue && (
          <span className="text-xs px-2.5 py-1 rounded-full border border-[var(--color-border)] text-[var(--color-text-secondary)]">
            {paper.venue}
          </span>
        )}
      </div>
      ...
      <h1 className="text-2xl md:text-3xl font-bold text-[var(--color-text-primary)] leading-snug mb-4"
        ...
        {paper.system_name && (
          <span className="text-[var(--color-accent)] font-mono mr-2">[{paper.system_name}]</span>
        )}
        {paper.title}
      </h1>
      ...
      <p className="text-sm text-[var(--color-text-secondary)] mb-6 pl-5">
        {paper.authors.split(/, | and /).map((author, i, arr) => (
          <span key={i}>
            <span
              className="hover:text-[var(--color-accent)] cursor-pointer transition-colors underline decoration-dotted decoration-[var(--color-border)] underline-offset-2"
      ...
      <div className="flex flex-wrap gap-1.5 mb-8 pl-5">
        {paper.codeUrl && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--color-accent)]/10 text-[var(--color-accent)]">Open Source</span>
        )}
        {paper.slidesUrl && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--color-blue)]/10 text-[var(--color-blue)]">Slides</span>
        )}
        {paper.talkUrl && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--color-purple)]/10 text-[var(--color-purple)]">Talk Video</span>
        )}
        {exp?.fine_tuning && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-[var(--color-orange)]/10 text-[var(--color-orange)]">Fine-tuned</span>
        )}
      </div>
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            {exp.fuzzer && exp.fuzzer.length > 0 && (
              <div className="bg-[var(--color-bg-card)] px-4 py-3 rounded-xl">
                <div className="text-xs text-[var(--color-text-muted)] mb-1">Fuzzer</div>
                <div className="text-sm text-[var(--color-text-secondary)] font-medium">{exp.fuzzer.join(', ')}</div>
              </div>
            )}
            {exp.static_tool && exp.static_tool.length > 0 && (
              <div className="bg-[var(--color-bg-card)] px-4 py-3 rounded-xl">
                <div className="text-xs text-[var(--color-text-muted)] mb-1">Static Tools</div>
                <div className="text-sm text-[var(--color-text-secondary)] font-medium">{exp.static_tool.join(', ')}</div>
              </div>
            )}
            ...
            {exp.benchmark_size && (
              <div className="bg-[var(--color-bg-card)] px-4 py-3 rounded-xl">
                <div className="text-xs text-[var(--color-text-muted)] mb-1">Benchmark Size</div>
                <div className="text-sm text-[var(--color-text-secondary)] font-medium">{exp.benchmark_size}</div>
              </div>
            )}
            {exp.cost && (
              <div className="bg-[var(--color-bg-card)] px-4 py-3 rounded-xl">
                <div className="text-xs text-[var(--color-text-muted)] mb-1">Cost</div>
                <div className="text-sm text-[var(--color-text-secondary)] font-medium">{exp.cost}</div>
              </div>
            )}
          </div>
        </section>
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {rp.categories.map(cat => (
                    <span key={cat} className="text-xs" style={{ color: categoryColors[cat] }}>
      ...
      <div className="flex items-center justify-between pt-8 border-t border-[var(--color-border)]">
        <button
          onClick={onBack}
          className="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-accent)] transition-colors bg-transparent border-none cursor-pointer"
        >
          ← Back to papers
        </button>
        <div className="flex items-center gap-3">
          {onPrev && (
            <button onClick={onPrev} className="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-accent)] transition-colors bg-transparent border-none cursor-pointer">
              ← Previous
            </button>
          )}
          {onNext && (
            <button onClick={onNext} className="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-accent)] transition-colors bg-transparent border-none cursor-pointer">
              Next →
            </button>
          )}
        </div>
